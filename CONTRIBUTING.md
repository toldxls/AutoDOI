...
- `tests/browser/match-bench.js` (`npm run bench:match`) is the yardstick for any change to `matchConfidence` or the page's `resolve()`: green precision must stay at 100% on the deposited variant and recall should not fall. Its `glued` variant is the yardstick for in-line splitting (`gluePoint` in `index.html`), together with the "no single real reference is split" check in `tests/splitter-bench.test.js`. `tests/matching.test.js` pins the cases it has already taught us.
