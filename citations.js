...
(function (root) {
   ...
   *   .. subscript, .. superscript, .. italic (e.g. taxa).
  ...
  var SUBO = '', SUBC = '', SUPO = '', SUPC = '', ITO = '', ITC = '';
  var MARKS_RE = /[-]/g, HAS_MARK = /[-]/;
  ...
  // MathML children at the top level of an element's content
  function mmlChildren(xml) {
    var out = [], depth = 0, start = -1, re = /<(\/?)mml:\w+[^>]*?(\/?)>/g, m;
    while ((m = re.exec(xml))) {
      if (m[2]) { if (depth === 0) out.push(m[0]); continue; }
  ...
  function mathmlToMarks(s) {
    return s.replace(/<mml:math[\s\S]*?<\/mml:math>/g, function (math) {
      math = math.replace(/>\s+</g, '><');
      var inner = /<mml:(msub|msup|msubsup)>((?:(?!<mml:(?:msub|msup|msubsup)>)[\s\S])*?)<\/mml:\1>/;
      var guard = 0;
      while (inner.test(math) && guard++ < 50) {
        math = math.replace(inner, function (all, kindName, body) {
          var ch = mmlChildren(body).map(mmlText);
          if (kindName === 'msub') return (ch[0] || '') + SUBO + (ch[1] || '') + SUBC;
          if (kindName === 'msup') return (ch[0] || '') + SUPO + (ch[1] || '') + SUPC;
          return (ch[0] || '') + SUBO + (ch[1] || '') + SUBC + SUPO + (ch[2] || '') + SUPC;
        });
      }
      return mmlText(math);
    });
  ...
  function markup(html) {
    ...
    var s = String(html);
    if (/<mml:/.test(s)) s = mathmlToMarks(s);
    ...
    s = clean(s).replace(/&delta;/gi, 'δ').replace(/&Delta;/g, 'Δ');
    return s.replace(/||/g, '');
  }

  // One formula such as Mg2SiO4, (Mg,Fe)2SiO4, Ca3Zr2[Fe2SiO12], Fe3+, SO42−, NH4+ -> marked string, or null
  function parseFormula(s) {
    var i = 0, out = '', depth = 0, elems = 0, groups = 0, two = false, oh = false, paren = false, bad = false;
    var lastCount = null; // {start, digits} of the most recent subscript, for charge handling
    function count() {
      var m = s.slice(i).match(/^\d+(?:\.\d+)?/);
      if (!m) { lastCount = null; return; }
      if (m[0] === '1' || (m[0].indexOf('.') === -1 && Number(m[0]) >= 100)) bad = true; // formulas never write 1; 9001 is not a count
      lastCount = { start: out.length, digits: m[0] };
      out += SUBO + m[0] + SUBC; groups++; i += m[0].length;
    }
    while (i < s.length) {
      var rest = s.slice(i), m = rest.match(/^[A-Z][a-z]?/);
      if (m) {
        var sym = m[0];
        if (!ELEM[sym]) { if (sym.length === 2 && ELEM[sym[0]]) sym = sym[0]; else return null; }
        out += sym; i += sym.length; elems++;
        if (sym.length === 2) two = true;
        if (sym === 'O' || sym === 'H') oh = true;
        count(); continue;
      }
      var c = rest[0];
      if (c === '(' || c === '[') { out += c; depth++; paren = true; i++; lastCount = null; continue; }
      if ((c === ')' || c === ']') && depth > 0) { out += c; depth--; i++; count(); continue; }
      if (c === ',' && depth > 0) { out += ','; i++; if (s[i] === '') { out += ' '; i++; } continue; }
      break;
    }
    if (depth !== 0 || !elems) return null;
    var rest2 = s.slice(i), charge = '';
    if (rest2) {
      var cm = rest2.match(/^([+−]{1,2}|-{1,2})$/);
      if (!cm) return null;
      var sign = cm[1];
      if (sign[0] === '-' && !lastCount) return null; // "Mg-" is a suspended hyphen ("Mg- and Fe-rich"), not a charge
      if (lastCount && lastCount.start + SUBO.length + lastCount.digits.length + SUBC.length === out.length) {
        var d = lastCount.digits, head = out.slice(0, lastCount.start);
        if (elems === 1 && d.indexOf('.') === -1 && d.length === 1) { out = head + SUPO + d + sign.replace(/-/g, '−') + SUPC; groups--; }
        else if (d.length >= 2 && d.indexOf('.') === -1) { out = head + SUBO + d.slice(0, -1) + SUBC + SUPO + d.slice(-1) + sign.replace(/-/g, '−') + SUPC; }
        else out += SUPO + sign.replace(/-/g, '−') + SUPC;
      } else out += SUPO + sign.replace(/-/g, '−') + SUPC;
      charge = sign;
    }
    if (bad && !charge) return null;
    if (/^H\d{1,2}N\d{1,2}$/.test(s)) return null;                 // influenza subtypes
    if (charge) {
      if (/^[BC]\+*$/.test(s.replace(/[+−-]/g, '+'))) return null; // B+, C++ (grades, languages)
      return out;
    }
    if (elems < 2 || !groups) return null;
    if (!(groups >= 2 || two || oh || paren)) return null;          // "IPCC5", "W3C" are not formulas
    return out;
  }

  function isotope(p) { // 40Ar, 18O, 87Sr, 3He, 206Pb, optionally charged
    var m = p.match(/^(\d{1,3})([A-Z][a-z]?)$/);
    if (!m || !ELEM[m[2]]) return null;
    var a = Number(m[1]), z = ELEM[m[2]];
    if (a < z || a > 2.7 * z + 3) return null;                      // 4K, 3C, 2F are not isotopes
    if (m[2] === 'S' && (a < 32 || a > 36)) return null;            // 16S / 40S are rRNA and ribosomes
    return SUPO + m[1] + SUPC + m[2];
  }

  function formulaPiece(p, allowCoef) {
    if (!p) return null;
    var coef = '';
    if (allowCoef) { var cm = p.match(/^(\d+|[nx])(?=[A-Z(])/); if (cm) { coef = cm[1]; p = p.slice(coef.length); } }
    var pre = '';
    var cn = p.match(/^\[(\d{1,2})\]/);                           // mineral site coordination: [4]Fe3+
    if (cn) { pre = SUPO + '[' + cn[1] + ']' + SUPC; p = p.slice(cn[0].length); }
    if (/^[δΔεΣ]/.test(p)) { pre += p[0]; p = p.slice(1); }  // δ18O, Σ
    var iso = isotope(p);
    if (iso) return coef + pre + iso;
    var f = parseFormula(p);
    if (f) return coef + pre + f;
    return pre && pre.indexOf(SUPO) !== -1 && /^[A-Z]/.test(p) ? coef + pre + p : null;
  }

  function formulaWord(w) {
    var lead = '', trail = '', core = w, changed = true;
    var opens = function (x) { return (x.match(/[(\[{]/g) || []).length; }, closes = function (x) { return (x.match(/[)\]}]/g) || []).length; };
    while (changed) {
      changed = false;
      ...
      if (/^[(\[{]/.test(core) && opens(core) > closes(core)) { lead += core[0]; core = core.slice(1); changed = true; }
    ...
    var any = false;
    var ratio = core.split('/').map(function (part) {
      var hyd = part.split(/([·•⋅])/).map(function (h, hi) {
        if (hi % 2) return h;
        return h.split(/(-(?=[A-Za-z]))/).map(function (piece, pi) {
          if (pi % 2) return piece;
          var r = formulaPiece(piece, hi > 0);
    ...
    return any ? lead + ratio + trail : null;
  }

  function autoFormulas(s) {
    if (!s) return s;
    // "(Mg, Fe)SiO3": keep the element list together while splitting words
    s = s.replace(/\((?:[A-Z][a-z]?,\s+)+[A-Z][a-z]?\)/g, function (g) { return g.replace(/,\s+/g, ','); });
    return s.split(/(\s+)/).map(function (w) {
      if (!w || /^\s+$/.test(w) || HAS_MARK.test(w)) return w;
      var f = formulaWord(w);
      return f === null ? w : f;
    }).join('').replace(//g, ' ');
  }

  // Carry deposited markers onto a title whose letters were re-cased (sentence case)
  function transferMarks(marked, plain) {
    if (stripMarks(marked).toLowerCase() !== String(plain).toLowerCase()) return plain;
    var out = '', j = 0;
    for (var i = 0; i < marked.length; i++) {
      var ch = marked[i];
      if (HAS_MARK.test(ch)) out += ch; else out += plain[j++];
    }
    return out;
  }
  ...
  var SUP_MAP = { '0': '⁰', '1': '¹', '2': '²', '3': '³', '4': '⁴', '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹', '+': '⁺', '-': '⁻', '−': '⁻', '=': '⁼', '(': '⁽', ')': '⁾', '[': '[', ']': ']', 'n': 'ⁿ', 'i': 'ⁱ' };
  ...
  function normalize(m) {
    ...
    var r = {
      ...
      titleMarked: trimPunct(markup(first(m.title))),
