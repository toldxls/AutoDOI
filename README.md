...
`tests/browser/match-bench.js` measures reference matching against 152 references as publishers deposited them in Crossref reference lists, each with the DOI it resolved to, plus lowercased and year-less variants; it drives the real page against the real services and reports green precision and recall (`npm run bench:match`).
