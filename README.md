...
2. **Find a DOI.** Title plus optional journal. Best matches come back with a match chip; *Format* sends one to the first tab, and a hit whose title matches goes there by itself. The record you are working with follows you across the tabs: a DOI looked up on the first tab prefills the title and journal here and the References box on the third tab (selected on focus, so typing replaces it), and a matched reference on the third tab replaces the example on the first.
